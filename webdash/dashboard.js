...
// --- Box plot by group -------------------------------------------------
function plotBox(elId, rows, groupCol, valueCol, title) {
  if (!rows.length) return renderEmpty(elId);
  const buckets = {};
  rows.forEach(r => {
    const k = r[groupCol];
    ...
    if (r[valueCol] != null && isFinite(r[valueCol])) buckets[k].push(r[valueCol]);
  });
  // Sort groups by median descending
  const groups = Object.keys(buckets).sort((a, b) => median(buckets[b]) - median(buckets[a]));
  const traces = groups.map((g, i) => ({
    type: "box", y: buckets[g], name: g,
    marker: { color: ACCENT_COLORS[i % ACCENT_COLORS.length] },
    boxpoints: false,
  }));
  Plotly.newPlot(elId, traces, commonLayout({
    title: { text: title || `${valueCol} by ${groupCol}`, x: 0.5 },
    xaxis: { tickangle: -30 },
    yaxis: { title: valueCol, gridcolor: "#EEE" },
    height: 400,
    showlegend: false,
  }), PLOTLY_CONFIG);
}
...
function marginSummaryRows(rows, groupCol) {
  const out = [];
  unique(rows, groupCol).sort().forEach(name => {
    const sub = rows.filter(r => r[groupCol] === name);
    if (!sub.length) return;
    const margins = sub.map(r => r.ProfitMargin).filter(v => v != null && isFinite(v));
    const lossOrders = countWhere(sub, r => r.Profit < 0);
    out.push({
      [groupCol]: name,
      "Median Margin %": +median(margins).toFixed(1),
      "P25 Margin %": +quantile(margins, 0.25).toFixed(1),
      "P75 Margin %": +quantile(margins, 0.75).toFixed(1),
      "Loss Orders": lossOrders,
      "Loss-order %": +((lossOrders / sub.length) * 100).toFixed(1),
      Orders: sub.length,
      Revenue: +sumBy(sub, "SaleTotal").toFixed(2),
    });
  });
...
function pageExec(rows) {
  const [thisRows, lastRows] = splitYears(rows);
...
function pageAffPerf(rows) {
  ...
  const [tyA, lyA] = splitYears(affRows);
...
function pageH2H(_rows) {
  ...
  const [aTy, aLy] = splitYears(aRows);
  const [bTy, bLy] = splitYears(bRows);
  return `
...
function pageH2HPost(_rows) {
  ...
  const [aTy] = splitYears(STATE.rows.filter(r => r.CompanyName === A));
  const [bTy] = splitYears(STATE.rows.filter(r => r.CompanyName === B));
...
function pageSportPost(rows) {
  const [thisRows] = splitYears(rows);
...
function pageTeamPost(_rows) {
  ...
  const [thisWork] = splitYears(work);
...
function pageMarketplace(rows) {
  ...
  return `
    ...
    <h3 class="section-h">Margin distribution by marketplace (TY)</h3>
    <div id="mkt-margin-box" class="plot"></div>
    <details class="data-expander">
...
function pageMarketplacePost(rows) {
  ...
  plotBox("mkt-margin-box", thisRows, "ShippingCompany", "ProfitMargin", "Margin distribution by marketplace");
...
function pageMargin(rows) {
  const [thisRows, lastRows] = splitYears(rows);
  const groups = STATE.affiliate === "__ALL__"
    ? MARGIN_GROUPS
    : MARGIN_GROUPS.filter(g => g[1] !== "CompanyName");
  const grp = ps("marginGroup", groups[0][1]);
  const validGrp = groups.find(g => g[1] === grp) ? grp : groups[0][1];
  const opts = groups.map(([label, val]) =>
    `<button class="metric-pill ${val === validGrp ? "active" : ""}" data-grp="${val}">${label}</button>`
  ).join("");
  return `
    ...
    <div class="picker-row">
      <span class="metric-label">Group margin by:</span><div id="margin-grp">${opts}</div>
    </div>
    <div class="two-col">
      <div><h3 class="section-h">Cost vs Revenue (TY)</h3><div id="margin-scatter" class="plot"></div></div>
      <div><h3 class="section-h" id="margin-box-title">Margin by group (TY)</h3><div id="margin-box" class="plot"></div></div>
    </div>
    <h3 class="section-h">Loss-making orders (top 20 by loss size)</h3>
    <div id="margin-losses"></div>
    <details class="data-expander">
      <summary>📋 Margin summary by group</summary>
      <div class="two-col">
        <div><h4>By Affiliate</h4><div id="margin-summary-aff"></div></div>
        <div><h4>By Sport</h4><div id="margin-summary-sport"></div></div>
      </div>
    </details>
  `;
}
function pageMarginPost(rows) {
  ...
  const grp = ps("marginGroup", groups[0][1]);
  const validGrp = groups.find(g => g[1] === grp) ? grp : groups[0][1];
  const grpLabel = MARGIN_GROUPS.find(g => g[1] === validGrp)[0];
  document.getElementById("margin-box-title").textContent = `Margin by ${grpLabel} (TY)`;
  plotBox("margin-box", thisRows, validGrp, "ProfitMargin", null);
  ...
  document.getElementById("margin-summary-aff").innerHTML =
    buildTable(marginSummaryRows(thisRows, "CompanyName"), [
      { key: "CompanyName", label: "Affiliate", type: "text" },
      { key: "Revenue", label: "Revenue", type: "money" },
      { key: "Median Margin %", label: "Median Margin %", type: "pct", gradient: "highGood" },
      { key: "P25 Margin %", label: "P25 Margin %", type: "pct" },
      { key: "P75 Margin %", label: "P75 Margin %", type: "pct" },
      { key: "Loss Orders", label: "Loss Orders", type: "int" },
      { key: "Loss-order %", label: "Loss-order %", type: "pct", gradient: "lowGood" },
      { key: "Orders", label: "Orders", type: "int" },
    ]);
  document.getElementById("margin-summary-sport").innerHTML =
    buildTable(marginSummaryRows(thisRows, "Sport"), [
      { key: "Sport", label: "Sport", type: "text" },
      { key: "Revenue", label: "Revenue", type: "money" },
      { key: "Median Margin %", label: "Median Margin %", type: "pct", gradient: "highGood" },
      { key: "P25 Margin %", label: "P25 Margin %", type: "pct" },
  ...
  document.querySelectorAll("#margin-grp .metric-pill").forEach(btn => {
    btn.addEventListener("click", () => setPs("marginGroup", btn.dataset.grp));
  });
}
...
function pageTicket(rows) {
  ...
  return `
    ...
    <h3 class="section-h">Margin distribution by ${tierLabel} (TY)</h3>
    <div id="ticket-margin-box" class="plot"></div>
    <details class="data-expander">
...
function pageTicketPost(rows) {
  ...
  plotBox("ticket-margin-box", thisRows, tier, "ProfitMargin", null);
...
function pageTime(rows) {
  ...
  return `
    ...
    <h3 class="section-h">Margin by lead-time bucket (TY)</h3>
    <div id="time-leadbox" class="plot"></div>
  `;
}
function pageTimePost(rows) {
  ...
  plotBox("time-leadbox", thisRows, "LeadTimeBucket", "ProfitMargin", null);
...
function renderPage() {
  ...
  if (!def) return;
  document.getElementById("main-content").innerHTML = def.html(rows);
  if (def.post) def.post(rows);
}
...
function buildSidebar() {
  ...
  document.querySelectorAll("#sport-filter .sport-pill").forEach(btn => {
    btn.addEventListener("click", () => {
      ...
      const s = btn.dataset.sport;
      if (set.has(s)) set.delete(s); else set.add(s);
      // null = no filter (all selected); otherwise the chosen subset
